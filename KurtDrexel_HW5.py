#by Kurt Drexel, May 2024
#call this program using: python KurtDrexel_HW5.py inputdirectoryname outputdirectoryname
#the clustering will be done at the end
    ...
    with open(outputfile, 'w') as file:#open output file
        for token in ProcessedTok[DocNum]:#iterate through tokenized words
            try:
                ...
                pass
#This is where hw5 begins 

#was unable to find a effective way to get information from output files so i decided to create my own function to achieve this
def read_tfidf_from_files(output_directory):#function to read contents of output files
    tfidf_weights = []#store tfidf weights
    filenames = sorted(os.listdir(output_directory))
    for filename in filenames:#iterate through files
        ...
        with open(file_path, 'r') as file:#open each file
            ...
            doc_weights = {}#hold weights from each each document 
            for line in lines:#iterate through lines in each file
                token, weight = line.strip().split('     |         ')#split weight and term
                doc_weights[token] = float(weight)  # Convert weight to float
            tfidf_weights.append(doc_weights)  #put weights as dictionaries
    return tfidf_weights

tfidf_weights = read_tfidf_from_files(output_directory)#get weights and terms from files
tokens = set().union(*(weights.keys() for weights in tfidf_weights))#this was able to work, making me confused
#tokens = set(weights.keys() for weights in tfidf_weights)#this did not work for a unknown reason
tfidf_matrix = np.array([[doc.get(token, 0) for token in tokens] for doc in tfidf_weights])#matric for weights
similarity_matrix = np.array([[1 - cosine(doc1, doc2) for doc2 in tfidf_matrix] for doc1 in tfidf_matrix])#get similarity matrix compairng doc 1 to doc2
#doc2 iterates through all documents comparing to doc1, once iterate through all docs doc1 is incremented to next document 
linkage_matrix = linkage(similarity_matrix, method='average')#linage matrix
clusters = fcluster(linkage_matrix, t=0.4, criterion='distance')#create clusters
#create dendrogram
plt.figure(figsize=(10, 6))#size of graph
dendrogram(linkage_matrix, truncate_mode='level', p=15)#dendrogram funct
plt.xlabel('Documents')#x axis
plt.ylabel('Distance')#y axis
plt.title('Hierarchical Clustering Dendrogram')#graph title
plt.axhline(y=0.4, color='r', linestyle='--', label=f'Distance Threshold: {0.4}')
plt.legend()
plt.show()

cluster_dict = {}#dictionary to hold all clusters
for idx, cluster_id in enumerate(clusters):
    doc_id = idx
    if cluster_id not in cluster_dict:#if cluster not in dict
        cluster_dict[cluster_id] = [doc_id]
    else:#add clster to dict, new cluster found 
        cluster_dict[cluster_id].append(doc_id)

print("\nClustering Results:")#print clusters
for cluster_id, docs in cluster_dict.items():
    doc_ids = [f"{doc_id + 1:03d}.txt" for doc_id in docs]
